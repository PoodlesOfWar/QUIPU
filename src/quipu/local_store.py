...
# pandas is only needed by the fetch_* DataFrame helpers below; import it lazily
# (inside those functions) so lightweight callers — e.g. a sibling repo borrowing
# the Brain's LLM router/ensemble, which only needs db_path() from this module —
# can import brain.local_store without the full analytics stack installed.
if TYPE_CHECKING:
    import pandas as pd
...
@contextmanager
def _conn():
    cn = sqlite3.connect(str(db_path()), timeout=20)
    cn.execute("PRAGMA journal_mode=WAL")
    cn.execute("PRAGMA synchronous=NORMAL")
    cn.execute("PRAGMA wal_autocheckpoint=100")  # checkpoint every ~400 KB; default 1000 let WAL grow unbounded
    try:
        yield cn
        cn.commit()
    finally:
