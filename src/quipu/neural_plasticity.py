...
_DEFAULT_DIALS: dict[str, dict[str, float]] = {
    ...
    # Perception Mk1 — visual / embodied reasoning sense
    "perception": {
        "max_images_per_round":  4.0,   # images processed per corpus round
        "confidence_threshold":  0.40,  # min VLM confidence to log as learning
        "scan_depth":            1.0,   # 0=docs only, 1=docs+data/documents
    },
}
...
def compute_capability_targets(state: dict[str, float]) -> dict[str, dict[str, float]]:
    ...
    return {
        ...
        # Perception Mk1 — visual sense dials grow with corpus richness.
        # As the knowledge graph matures, Perception processes more images
        # per round and lowers the confidence bar (trusts VLMs more when
        # ensemble weights are well-trained).
        "perception": {
            # Throughput: start at 4 images/round, grow to 16 at full richness.
            "max_images_per_round": lerp(4.0,   16.0,  richness),
            # Confidence threshold relaxes as the vision model weights stabilise
            # through ADAM learning (parallels how Vision thresholds relax with
            # entity count).
            "confidence_threshold": lerp(0.40,  0.25,  s_rnd),
            # scan_depth expands from docs-only (1) to docs+data (1) →
            # all assets (2) as the corpus can absorb the new entities.
            "scan_depth":           lerp(1.0,   2.0,   s_doc),
        },
    }
