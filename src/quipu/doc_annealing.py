...
def render_system_map(collected: dict, map_version: int, fingerprint: str) -> str:
    ...
    L.append("| `divine_blessing.py` | `DIVINE_BLESSING_SQRT(-1)` — attestation store; routes every Entirety write path through the six gates |")
    L.append("")
    L.append("**`qpsi/` — governance and learning dynamics** "
             "(see `docs/QPSI_GOVERNANCE.md`; every path holds until a human places a key and a grant)")
    L.append("")
    L.append("| Module | Role |")
    L.append("|---|---|")
    L.append("| `qpsi/cat_residual.py` | Complex CAT amplitudes, residual operator, rectifier, Lipschitz clamp, edge proposals |")
    L.append("| `qpsi/weyl_channel.py` | Ricci/Weyl split; sparsest exact trace-free event decomposition |")
    L.append("| `qpsi/edge_gate.py` | Displacement-gated edge upsert carrying the flip count and cost class |")
    L.append("| `qpsi/governance.py` | Six Physical Gates, attestation assurance, `authorised_to_realise` |")
    L.append("| `qpsi/residual_checkpoint.py` | Reference advances only on realisation; held residual, log, rollback, release |")
    L.append("| `qpsi/emergence_detector.py` | Parity-locked emergence, r-ADMIN recognition, the 翈 Signature |")
    L.append("")
