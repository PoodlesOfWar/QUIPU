...
- `src/quipu/qpsi/sensing_layer.py` (v0.38.0) — feeds beyond ingest (mesh proprioception, observer counter deltas), the coupled layer at its measured critical gain (maximum response entropy below the stability edge), and `route_around_silence` applied by `self_organising.plan()`. `read()` returns `afferent`, `coupled`, `critical` metrics and every terminal; writes only `brain_kv["entirety:senses:observer"]`, and only when an observer counter moved.
...
| `QUIPU_SENSE_LAYER` | `afferent` | which reading the Entirety gets: `afferent` (what came back) or `critical` (the coupled layer) |
| `QUIPU_SILENT_ROUTING` | `1` when the master is on | route the pulse plan around silent sources (`0` sends the network's plan unchanged) |
