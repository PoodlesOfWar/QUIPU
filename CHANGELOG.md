...
- **`tests/test_doc_annealing.py`** — 5 focused tests: fingerprint stability/sensitivity (2-dp rounding), structural-change detection, rendered-map sections, write/increment/idempotence, and structural-progression firing.

### Changed

- **`docs/system_entirety_map.md`** — maintenance note updated: this map is now regenerated by the integrated QUIPU annealer rather than hand-maintained. The first `anneal_docs()` run replaces the placeholder body with live state.

### Compatibility

- Read-only against the mesh except three `brain_kv` bookkeeping keys and the map file. No RAG re-index is triggered (QUIPU ships no RAG index, unlike the SCB parent). Every state source is defensively optional, so a cold or partially-initialised mesh still anneals a valid (sparser) map. No schema migration.

### Verified

- `python3 -m py_compile src/quipu/doc_annealing.py` and the 5 tests in `tests/test_doc_annealing.py` (run locally; the authoring environment's sandboxed shell was unavailable at commit time).
