...
## Where the model stands (2026-07-23)

Version 0.25.0. The STP-style geodesic diagnostic is live in `train_round()` behind `QUIPU_STP_DIAGNOSTIC` (default on, purely observational): each round records the Semantic-Tube-Prediction gap in both the learned 7-D embedding and an isometric ℝ⁴ torus embedding, with capped rolling histories in `mesh_slm_meta`. The P1 validation itself is **pending** — `stp_diagnostic_trend()` exists but needs dozens of stabilized rounds on real corpus data before the confirm/falsify result can be recorded here and Phase 2 (η coupling) considered.
